...
class RouteService:
    ...
    async def _load_alternate_route(self, source: str, destination: str) -> None:
        ...
        try:
            ...
            logger.debug("****************************************************")
            logger.debug(f"Received alternate route name: {alternate_route_name}")
    ...
    async def create_alternate_route_map(
        ...
        try:
            ...
            self.alternate_route = (
                None  # Clear alternate route data after map creation to free memory
            )
    ...
    def create_route_map(
        ...
        logger.debug("-----------------------------")
        logger.debug(f"Creating route map for: {start_location} to {end_location}")
        logger.debug(f"Available location coordinates: {incident_location}")
        ...
        if all_routes or incident_location:
            logger.debug(
                f"Adding intersection markers for incident location: {incident_location} and all routes: {all_routes}"
            )
